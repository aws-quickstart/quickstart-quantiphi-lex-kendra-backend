...
client_sqs = boto3.client('sqs','us-east-1')
...
def handler(event, context):
    if 'RequestType' in event and event['RequestType'] == 'Delete':
        try:
            response_lambda = client.get_function(
            FunctionName='KendraDataSyncOperationsFunctiontest'
            )
            data = json.load(response_lambda)
            ...
            response_ssm_faq_id = client_ssm.get_parameter(
                Name='/dev/index/dev/index/kendra-faq-id'
            )
        ...
        try:
            # response_lambda = client.get_function(
            # FunctionName='KendraOperationsFunctiontest'
            # )
            ...
            if response_kendra['Status'] == 'CREATING':
                ...
                if 'Contents' in response_s3 and len(response_s3['Contents']) > 0:
                    ...
                    s3.put_object(Bucket='lex-kendra-code',Key='status_sync_count.json',Body = serializedMyData)
