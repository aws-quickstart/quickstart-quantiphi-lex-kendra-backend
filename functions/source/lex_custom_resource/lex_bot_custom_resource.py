...
def create_lex_intents(fulfillment_lambda, intents):
    """
    Creates Lex intents.
...
def create_lex_bot(lex_bot, fulfillment_lambda):
    """
    Creates Lex Bot.
    ...
    if 'intents' in lex_bot:
        intent_list = create_lex_intents(fulfillment_lambda, lex_bot['intents'])
...
def create(event, _):
    ...
    bot_name = create_lex_bot(lex_json['resource'],
                              event['ResourceProperties']['FulfillmentLambda'])
...
def delete_intents(bot_name, intents):
    ...
    :return: List of slot types associated with delete indents
    """
    slot_types = set()
    for intent in intents:
        try:
